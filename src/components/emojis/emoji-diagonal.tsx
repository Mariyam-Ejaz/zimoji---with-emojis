...
// import Worker from 'worker-loader!@/utils/emoji-extractor'; // Adjust the path as needed
// import { EmojiWorker } from '@/utils/emoji-extractor'; // Assuming this is your worker type
...
//   const emojiWorker = wrap<EmojiWorker>(new Worker());

//   // Fetch emoji URLs from the worker
//   useEffect(() => {
//     const fetchEmojis = async () => {
//       try {
//         const emojiUrls = await emojiWorker.getEmojiUrls(); // Make sure this method exists in the worker
//         setEmojis(emojiUrls.map((url) => ({ src: url, top: 0, left: 0 }))); // Initialize positions later
//       } catch (error) {
...
//   // Generate emoji positions and set up sampling area
//   useEffect(() => {
//     if (emojis.length === 0) return; // Wait for emojis to load
...
//     const newEmojis: EmojiProps[] = points.map((point, index) => ({
//       src: emojis[index % emojis.length].src, // Get emoji src from the state
//       left: point[0],
//       top: point[1],
//     }));
...
//     setEmojis(newEmojis);
//     setVisibleEmojiCount(0);
//     setAllEmojisVisible(false);
//   }, [emojiSize, minDistance, refreshKey, emojis]);
